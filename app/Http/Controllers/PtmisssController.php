...
class PtmisssController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $ptmiss -> setTable('爱尔兰_ptmiss_tbl');
        $ptmissp= $ptmiss->find($id);
       // $miss= Miss::find($id);
       // if(auth()->user()->id!=$miss->user_id){
        //dd($missp->uname);
        if(auth()->user()->id!=$ptmissp->user_id){
            //need to confirm if '/posts'
            return redirect('/')->with('error','unathorized');
        }

        return view('ptmisss.ptmiss_edit')->with('ptmiss',$ptmissp)->with('ucountry',$ucountry);
    ...
    public function update(Request $request, $id)
    {
        ...
        return redirect('/ptmiss')->with('success', '资料修改成功！');
        }else{
            return redirect('/ptmiss')->with('fail', '你的资料已上传过了!');
        }
